...
export default function AdminRegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (success) {
        toast({ title: "Admin registration successful", description: "Welcome to FIR System Admin!" });
        navigate("/admin");
      } else {
    ...
    <div className="flex min-h-screen">
      <div className="hidden w-1/2 gradient-hero items-center justify-center lg:flex">
        <div className="max-w-md text-center px-8">
          ...
          <h2 className="mb-4 text-3xl font-bold text-white">Admin Registration</h2>
          <p className="text-white/60">Register as an administrator to manage the FIR system. Authorization code required.</p>
        </div>
      </div>
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <Input id="adminCode" type="password" value={adminCode} onChange={(e) => setAdminCode(e.target.value)} placeholder="Enter admin authorization code" required disabled={isSubmitting} />
            </div>
